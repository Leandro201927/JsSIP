...
module.exports = class RTCSession extends EventEmitter
{
  ...
  {
    /**
     * [CCORREA] En este punto crea un RTCPeerConnection, posiblemente intente hacer una oferta de codecs desde acá.
     */
    logger.debug('[CCORREA] _createRTCConnection:', pcConfig, rtcConstraints);
    this._connection = new RTCPeerConnection(pcConfig, rtcConstraints);
    this._transceivers = this._connection.getTransceivers();
    logger.debug('[CCORREA] _createRTCConnection->getTransceivers:', this._transceivers);

    if(this._transceivers.length === 0) {
      // [CCORREA]

      // Crear un transceptor para audio
      this._audioTransceiver = this._connection.addTransceiver('audio');
      // Obtener los transceptores de audio
      const audioTransceiver = this._connection.getTransceivers()[0]
      logger.debug('[CCORREA] _createRTCConnection->getTransceivers->audioTransceiver [1]', audioTransceiver);
      if(audioTransceiver.sender) {
        // Obtener formato correcto de codects
        const codecs = RTCRtpReceiver.getCapabilities('audio').codecs;
        // Obtener referencia de PCMA
        const pcmuCodec = codecs.find(codec => codec.mimeType === 'audio/PCMU' && codec.clockRate === 8000);
        const pcmaCodec = codecs.find(codec => codec.mimeType === 'audio/PCMA' && codec.clockRate === 8000);
        const g722Codec = codecs.find(codec => codec.mimeType === 'audio/G722' && codec.clockRate === 8000);
        if(pcmuCodec && pcmaCodec && g722Codec) {
          // Establecer preferencia de codec
          audioTransceiver.setCodecPreferences([pcmuCodec, pcmaCodec, g722Codec]);
          logger.debug('[CCORREA] _createRTCConnection->getTransceivers->codecs', codecs);
        }
      }
    }
    ...
    return Promise.resolve()
      ...
      .then(() =>
      {
        if (type === 'offer')
        {
           ...
           * [CCORREA] 
           */
          logger.debug('[CCORREA] _createLocalDescription', constraints)
